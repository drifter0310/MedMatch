...
const TRANSLATIONS = {
  en: {
    ...
    analyzing: "Analyzing chemical compound...",
    identifying: "Identifying alternatives...",
    scannedItem: "Scanned Item",
    genericCompound: "Generic Compound",
    cheaperAlts: "5 Cheaper Alternatives",
    ...
    about: "About MedMatch",
    signOut: "Sign Out",
    signUp: "Sign Up (Optional)",
    ...
    yesterday: "Yesterday",
    guest: "Guest User",
    guestDesc: "Sign up to sync across devices",
  ...
  bn: {
    ...
    snap: "ছবি তুলুন",
    upload: "গ্যালারি থেকে আপলোড",
    analyzing: "উপাদান বিশ্লেষণ করা হচ্ছে...",
    identifying: "বিকল্প খোঁজা হচ্ছে...",
    scannedItem: "স্ক্যান করা ওষুধ",
    genericCompound: "জেনেরিক উপাদান",
    cheaperAlts: "৫টি সস্তা বিকল্প",
    ...
    pharmacy: "পছন্দের ফার্মেসি",
    about: "MedMatch সম্পর্কে",
    signOut: "লগ আউট",
    signUp: "সাইন আপ (ঐচ্ছিক)",
    ...
    yesterday: "গতকাল",
    guest: "গেস্ট ইউজার",
    guestDesc: "ডিভাইস সিঙ্ক করতে সাইন আপ করুন",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    let storedId = localStorage.getItem('medmatch_user_id');
    if (!storedId) {
      storedId = 'anon_' + Math.random().toString(36).substring(2, 15);
      localStorage.setItem('medmatch_user_id', storedId);
    }
    setUserId(storedId);
  ...
  const saveScansToHistory = async (medicines: ScannedMedicine[]) => {
    for (const med of medicines) {
      const bestAlternative = med.alternatives && med.alternatives.length > 0
        ? med.alternatives.reduce((prev, current) => (prev.price < current.price) ? prev : current, med.alternatives[0])
        : null;
      const savedAmount = bestAlternative ? bestAlternative.savings : 0;
      ...
      try {
        ...
        if (res.ok) {
          ...
          setHistory(prev => [{ ...newScan, id: Date.now() + Math.random() }, ...prev]);
        }
      } catch (e) {
          ...
          if (res.ok) {
            ...
            if (Array.isArray(data.medicines)) {
              ...
              if (data.medicines.length > 0) {
                saveScansToHistory(data.medicines);
              }
            } else {
              setScannedMedicines([]);
              setScanState('results');
            ...
            >
              {scanState === 'idle' && (
                <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
                  ...
                  <p className="text-sm text-emerald-800/80 mb-6 max-w-[260px] leading-relaxed">
                    {t.desc}
                  </p>
                  ...
                  <button 
                    ...
                    className="w-full max-w-[280px] bg-emerald-600/90 hover:bg-emerald-700 backdrop-blur-md text-white rounded-2xl py-3.5 px-6 font-semibold text-base shadow-lg shadow-emerald-600/20 transition-transform active:scale-95 flex items-center justify-center gap-2 border border-emerald-500/50"
                  >
                  ...
                  <button 
                    ...
                    className="mt-3 w-full max-w-[280px] bg-white/60 backdrop-blur-md border border-white/80 hover:bg-white/80 text-emerald-900 rounded-2xl py-3.5 px-6 font-semibold text-base shadow-sm transition-colors active:scale-95 flex items-center justify-center gap-2"
                  >
                    ...
                    <div className="w-12 h-12 rounded-xl overflow-hidden border border-white/80 shrink-0 shadow-sm">
                      {imagePreview ? (
                        ...
                        <div className="w-full h-full bg-white/50 flex items-center justify-center">
                          <SplitCapsuleLogo className="w-5 h-5 text-emerald-600/50" />
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-3 space-y-4">
                    {scannedMedicines.length === 0 ? (
                      <div className="bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 p-6 text-center shadow-sm">
                        ...
                        <p className="text-sm text-emerald-800/80">We couldn't identify any medicines in this image. Please try again with a clearer photo.</p>
                      </div>
                    ) : (
                          ...
                          <div className="p-2 space-y-1.5">
                            ...
                            {med.alternatives?.map((alt, index) => (
                              <motion.div 
                                ...
                                key={index} 
                                className="bg-white/60 backdrop-blur-xl p-2 rounded-xl shadow-sm border border-white/80 flex items-center justify-between hover:bg-white/80 transition-colors"
                              >
                                <div className="flex-1">
                                  ...
                                  <div className="mt-1 flex items-center gap-2">
                                    ...
                                    <span className="text-[9px] font-bold text-emerald-700 bg-emerald-100/80 border border-emerald-200/50 px-1.5 py-0.5 rounded shadow-sm">
                                      {t.save} ৳ {(alt.savings || 0).toFixed(2)}
                                    </span>
                                  </div>
                                </div>
          ...
          {activeTab === 'history' && (
            ...
            >
              ...
              <div className="space-y-3">
                {history.length === 0 ? (
                  <div className="text-center text-emerald-800/60 py-10 text-sm font-medium">
                    No scans yet.
                  </div>
                ) : (
                  history.map((item, i) => (
            ...
            >
              ...
              <div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-sm border border-white/80 overflow-hidden mb-4">
                <div className="p-3 border-b border-white/40 flex items-center justify-between">
                  ...
                  {!email && (
                    <button onClick={() => setShowSignUp(true)} className="text-xs font-bold text-emerald-700 bg-white/50 px-3 py-1.5 rounded-full border border-white/60 hover:bg-white/80 transition-colors">
                      {t.signUp}
                    </button>
                  )}
                </div>
              ...
              <div className="bg-white/60 backdrop-blur-xl rounded-2xl shadow-sm border border-white/80 overflow-hidden">
                ...
                {[
                  ...
                  <div key={i} className="p-3 border-b border-white/40 last:border-0 flex items-center justify-between hover:bg-white/40 cursor-pointer transition-colors">
                    <span className="font-bold text-emerald-900 text-sm">{setting.label}</span>
                    <div className="flex items-center gap-1 text-emerald-700/80">
                      <span className="text-xs font-bold">{setting.value}</span>
                      <ChevronRight className="w-4 h-4" />
                    </div>
                  </div>
                ))}
              ...
              {email && (
                <button 
                  onClick={() => {
                    setEmail('');
                    localStorage.removeItem('medmatch_email');
                  }}
                  className="w-full mt-4 p-3 text-red-600 font-bold bg-white/60 backdrop-blur-xl border border-white/80 rounded-2xl hover:bg-red-50/80 transition-colors shadow-sm text-sm"
                >
                  {t.signOut}
                </button>
              )}
      ...
      <nav className="absolute bottom-0 w-full bg-white/40 backdrop-blur-xl border-t border-white/60 pb-[env(safe-area-inset-bottom)] z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <div className="flex justify-around items-center h-16 px-2">
          <button 
            ...
            className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${activeTab === 'dashboard' ? 'text-emerald-700' : 'text-emerald-900/40 hover:text-emerald-900/60'}`}
          >
            <Camera className={`w-5 h-5 ${activeTab === 'dashboard' ? 'fill-emerald-600/20' : ''}`} />
            <span className="text-[10px] font-bold">{t.scan}</span>
          </button>
          <button 
            onClick={() => setActiveTab('history')}
            className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${activeTab === 'history' ? 'text-emerald-700' : 'text-emerald-900/40 hover:text-emerald-900/60'}`}
          >
            <History className={`w-5 h-5 ${activeTab === 'history' ? 'fill-emerald-600/20' : ''}`} />
            <span className="text-[10px] font-bold">{t.history}</span>
          </button>
          <button 
            onClick={() => setActiveTab('settings')}
            className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${activeTab === 'settings' ? 'text-emerald-700' : 'text-emerald-900/40 hover:text-emerald-900/60'}`}
          >
            <Settings className={`w-5 h-5 ${activeTab === 'settings' ? 'fill-emerald-600/20' : ''}`} />
      ...
      <AnimatePresence>
        {showSignUp && (
          <motion.div 
            ...
            className="absolute inset-0 z-50 bg-emerald-950/20 backdrop-blur-sm flex items-center justify-center p-4"
            onClick={() => setShowSignUp(false)}
          >
            <motion.div 
              initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              className="bg-white/90 backdrop-blur-xl w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-white"
            ...
            >
              ...
              <form onSubmit={handleSignUp}>
                <input 
                  ...
                  className="w-full bg-white/50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
                />
                <div className="flex gap-2">
                  <button type="button" onClick={() => setShowSignUp(false)} className="flex-1 py-3 rounded-xl font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 transition-colors">
                    Cancel
                  </button>
                  <button type="submit" className="flex-1 py-3 rounded-xl font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-600/20">
                    {t.submit}
                  </button>
                </div>
              </form>
        ...
        {showPharmacyList && (
          <motion.div 
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute inset-0 z-50 bg-emerald-950/20 backdrop-blur-sm flex items-end justify-center"
            onClick={() => setShowPharmacyList(false)}
          >
            <motion.div 
              initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              transition={{ type: "spring", damping: 25, stiffness: 200 }}
              className="bg-white/90 backdrop-blur-xl w-full rounded-t-3xl p-6 shadow-2xl border-t border-white pb-[env(safe-area-inset-bottom)]"
              onClick={e => e.stopPropagation()}
            >
              ...
              <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-2">
                {PHARMACIES.map(pharmacy => (
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-emerald-50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
